...
- **Beber** (`POST /drink`): cobra **1 bronze sempre** (sem estamina). ⚠️ **[TAVERNA_CHANCE]** (2026-06-23) o **SERVIDOR sorteia o acerto** (o `success` do cliente é IGNORADO — anti-hack, senão dava p/ forjar buff de 100%). A **chance** começa em `DRINK_BASE_CHANCE` (90%) e **cai `DRINK_CHANCE_STEP`/stack** (−1%, piso `DRINK_MIN_CHANCE` 15%) = a **dificuldade que sobe**; chegar perto do cap fica quase impossível. **Acertar** → +1 stack + renova os 5 min; **errar** → só o bronze (não mexe stacks/timer → o buff pode decair). Buff **expirado** ao beber = **sessão nova** (zera stacks **e** `bottlesDrunk`). `drink` devolve `success` (front mostra copo **vazio**/**derramado**); `status` devolve `drinkChance` (barra verde do front). Front: 1 caneca PixelLab animada de 3 jeitos (`drink_tremble`/`drink_empty`/`drink_spill`), botão menor ao lado. O badge 🍺 do topbar faz countdown local e some ao expirar.
