...
class EconomicSinksIntegrationTest extends BaseIntegrationTest {
    ...
    // ── TC-245: custo de reforja = raridade³ × 500 [AUDITORIA A4] ──
    @Test
    @DisplayName("TC-245 | Custo de reforja = rarity³ × 500")
    void tc245_reforgeCostFormula() {
        Player p = playerOf("sink");
        p.addBronzeAmount(50_000);
        playerRepository.save(p);
        InventoryItem item = equippedItem(p, 4, 10, 0, 0, 100); // épico → 4³×500 = 32000

        assertThat(smithingService.reforgeCost(item)).isEqualTo(32_000);

        long before = playerRepository.findById(p.getId()).orElseThrow().totalBronze();
        smithingService.reforgeItem(playerRepository.findById(p.getId()).orElseThrow(), item.getId());
        long after = playerRepository.findById(p.getId()).orElseThrow().totalBronze();

        assertThat(before - after).isEqualTo(32_000);
    }
