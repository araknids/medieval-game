...
// [UNIFICAÇÃO_ZONA] A coleta roda via /api/zones; aqui testamos o roller por reino
// (collectGatheringDropsOnly) direto, que é o que o ZoneService usa.
...
class FishSplitTest extends BaseIntegrationTest {
    ...
    private int fishingLevel() {
        return gatheringService.getOrCreateSkill(player, SkillType.FISHING).getLevel();
    }
    ...
    void marAbencoado_yieldsHpFish() {
        List<GatheringService.ResourceDrop> drops =
                gatheringService.collectGatheringDropsOnly(SkillType.FISHING, fishingLevel(), 30, Kingdom.MAR_ABENCOADO);
    ...
    void aguasCalmas_yieldsStaminaFish() {
        List<GatheringService.ResourceDrop> drops =
                gatheringService.collectGatheringDropsOnly(SkillType.FISHING, fishingLevel(), 30, Kingdom.FISHING);
        assertThat(drops).isNotEmpty();
