...
async function loadWarrior() {
  ...
  document.getElementById('warrior-card').innerHTML = `
    ...
    ${(warrior.soulStones ?? 0) > 0 ? `
    <div style="font-size:.7rem;color:#a78bfa;margin-top:.3rem;font-weight:bold">
...
async function loadInventory() {
  ...
  if (slotEl && slots && !slots.error) {
    ...
    const barColor = pct >= 90 ? '#ef5350' : pct >= 70 ? '#ffc107' : '#4caf50';
    const expandBtn = !slots.inventoryExpanded
      ? `<button onclick="expandInventory()" style="font-size:11px;padding:3px 8px;background:#5b21b6;margin-left:8px">
           💎 Expand (3 SoulStones)
         </button>`
  ...
  el.innerHTML = `
    <div class="sk-section">
      ...
      ${data.soulStones > 0 ? (() => {
        ...
        const cdStr  = cdSecs > 0
          ? `${Math.floor(cdSecs/60)}m ${cdSecs%60}s`
          : '';
