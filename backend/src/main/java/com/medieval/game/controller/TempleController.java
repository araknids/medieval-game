...
public class TempleController {
    ...
    public ResponseEntity<?> heal(Authentication auth) {
        Player player = getPlayer(auth);
        // [HEAL_FIX] Curar SEMPRE conclui o dever HEAL — inclusive com HP já cheio (a "bênção" do Padre).
        // Antes havia 2 cálculos de HP (controller × heal()) que discordavam na fronteira 99–100% → heal()
        // lançava "HP cheio" e o onHealed nunca rodava (dever ficava pendente). [DIARIO_QUEST]
        boolean dutyPending = starterQuestService.isHealDutyPending(player);
        String msg;
        try {
            templeService.heal(player);
            msg = com.medieval.game.service.Messages.tr("msg.warrior_healed", "Warrior healed! HP restored to 100%.");
        } catch (IllegalStateException e) {   // já estava com HP cheio
            if (!dutyPending) throw e;        // sem dever pendente → erro normal "HP cheio"
            msg = com.medieval.game.service.Messages.tr("msg.priest_blessing",
                "Father Anselmo blesses you — you were already whole. Duty done.");
        }
        if (dutyPending) starterQuestService.onHealed(player);   // conclui o dever HEAL ao curar (sempre)
