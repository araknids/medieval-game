...
# [FIX] z NEGATIVO: o escudo vai pra parte EXTERNA da mão (lado de fora). -0.12 passava da mão (flutuava);
# -0.09 cola na mão. Tuning: x=mover de lado · y=ao longo da mão (cima/baixo) · z=externo(−)/frente(+). [ESCUDO_MAO]
const SHIELD_BASE := Vector3(0.0, 0.02, -0.09)
