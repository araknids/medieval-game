...
| A4 | ✅ | Reforja barata demais (re-roll ilimitado) | Economia | `service/SmithingService.java:258` | **RESOLVIDO:** custo `raridade²×200 → raridade³×500` (épico 3.200 → 32.000, ~10×). **Piso de qualidade (refino):** a reforja garante total ≥ **45%** do máximo da raridade (`REFORGE_FLOOR_PCT`, completa no HP até o piso) — protege itens de loja/craft de um re-roll ruim; `new Random()` → `ThreadLocalRandom`. Teste `a4_reforgeRespectsFloor`. |
