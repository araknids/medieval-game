...
| # | Status | Achado | Auditores | Local | Impacto / Correção |
|---|--------|--------|-----------|-------|--------------------|
| A1 | ⬜ | Gemas contam na ficha mas **não** no combate | Arquitetura | stats duplicados em `ArenaService:196` / `TowerService:135` / `ZoneService:542` / `WarriorController:70` | `totalGemBonus` só é somado no `WarriorController`. Jogador luta mais fraco do que a tela mostra. **Fix:** centralizar cálculo de stats num único `WarriorStatsCalculator` incluindo gemas/buffs. |
| A2 | ⬜ | `open-in-view=false` só em prod | Persistência | `application-prod.properties` | Dev/test rodam com `true` → regressão de lazy-loading passa nos testes e só explode em prod. **Fix:** `open-in-view=false` no `application.properties` base e/ou perfil de teste. |
| A3 | ⬜ | Renda escalável > sinks no late-game | Economia | `ZoneService.java:217`, `TowerService.java:157` | Zona COMBAT lv100 ≈ 3.750/h escalável; único sink que escala junto (reparo) ≈ 250–700/h. Inflação por cima. **Fix:** reduzir bronze da Zona (`level×8`), custo de subida na Torre (`floor×15`). |
| A4 | ⬜ | Reforja barata demais (re-roll ilimitado) | Economia | `service/SmithingService.java:258` | Épico = 3.200 fixo, re-roll até maximizar stats. **Fix:** `raridade³×500` ou custo crescente por uso; respeitar piso de stats de itens de loja/craft. |
