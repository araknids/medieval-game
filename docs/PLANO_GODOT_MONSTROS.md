...
**Mapa casa com o reino** ✅: o `_ready` agora faz `_setup_camera()` (cedo) → `await _load_events()` → `_setup_map()` (depois, já com a `scene`). `SCENE_TO_MAP` mapeia `arena→arena, tower→dungeon, coast→beach, sea→beach, cave→mining, fortress→castle`. Prioridade: `scenario` fixo (@export) > `scene` do backend > sorteio (mock/desconhecido). Números (tamanhos/hover/×1.25 boss) são placeholders p/ tuning.
